class MarkLottor extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (n = 0; n < maxballs; n++)
    {
      ...
      balls[n].setpos(random(0,360),0,0);  // theta,y,n/a
      balls[n].setvel(0,random(0.1,0.5),0);    // up speed
    }
...
public class MovObj
{
  ...
  MovObj(float x, float y, float z, float vx, float vy, float vz, color c)
  {
    setpos(x,y,z);
    setvel(vx,vy,vz);
    setcolor(c);
